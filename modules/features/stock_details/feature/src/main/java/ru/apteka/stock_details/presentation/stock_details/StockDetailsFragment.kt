package ru.apteka.stock_details.presentation.stock_details
...
import ru.apteka.stock_details.R
import ru.apteka.stock_details.databinding.StockDetailsFragmentBinding
...
class StockDetailsFragment : BaseFragment<StockDetailsViewModel, StockDetailsFragmentBinding>() {
    ...
    private val stock by lazy {
        requireArguments().get(STOCK_DETAILS_ARGUMENT) as StockModel
    }
    ...
    override fun onViewBindingInflated(binding: StockDetailsFragmentBinding) {
        ...
        binding.stockDetailsReadCompletely.setOnClickListener {
            if (binding.stockDetailsReadCompletely.text == getString(ComponentsR.string.read_completely)) {
        ...
        //TODO Вызывает ошибку при возврате из карточки продукта
