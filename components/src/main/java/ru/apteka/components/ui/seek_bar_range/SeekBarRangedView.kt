package ru.apteka.components.ui.seek_bar_range
...
class SeekBarRangedView @JvmOverloads constructor(
    ...
    var actionCallbacks: MutableList<SeekBarRangedChangeCallback> = mutableListOf()
...
fun SeekBarRangedView.removeActionListener(callback: SeekBarRangedView.SeekBarRangedChangeCallback) {
...
fun setMinValue(
    ...
    if (value != null) {
        seekBar.setMinValue(value.toFloat())
    }
}

@BindingAdapter("app:currentMinValue")
fun setCurrentMinValue(
    seekBar: SeekBarRangedView,
    value: Int?
) {
    if (value != null) {
        seekBar.selectedMinValue = value.toFloat()
    }
}

@InverseBindingAdapter(attribute = "app:currentMinValue", event = "currentMinValueChanged")
fun getCurrentMinValue(
    seekBar: SeekBarRangedView
) = seekBar.selectedMinValue.toInt()

@BindingAdapter("currentMinValueChanged")
fun setSelectedMinValueListener(
    seekBar: SeekBarRangedView,
...
fun setMaxValue(
    ...
    if (value != null) {
        seekBar.setMaxValue(value.toFloat())
    }
}

@BindingAdapter("app:currentMaxValue")
fun setCurrentMaxValue(
    seekBar: SeekBarRangedView,
    value: Int?
) {
    if (value != null) {
        seekBar.selectedMaxValue = value.toFloat()
    }
}

@InverseBindingAdapter(attribute = "app:currentMaxValue", event = "currentMaxValueChanged")
fun getCurrentMaxValue(
    seekBar: SeekBarRangedView
) = seekBar.selectedMaxValue.toInt()

@BindingAdapter("currentMaxValueChanged")
fun setSelectedMaxValueListener(
    seekBar: SeekBarRangedView,
    listener: InverseBindingListener?
) {
    if (listener != null) {
