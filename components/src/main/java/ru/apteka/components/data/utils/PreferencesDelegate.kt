...
/**
 * Делегат предпочтений.
 * @param preferences Экземпляр общих настроек.
 * @param name Имя
 * @param defValue Значение по умолчанию.
 * @param onDifficultTypeTransform Обработчик преобразования сложного типа данных в примитивный.
 * @param onPrimitiveTypeTransform Обработчик преобразования примитивного типа данных в сложный.
 * @param prefFlow Flow предпочтения.
 */
class PreferencesDelegate<DIFFICULT_TYPE, SIMPLE_TYPE>(
    ...
    operator fun getValue(thisRef: Any?, property: KProperty<*>) = getValueHandler()

    operator fun setValue(thisRef: Any?, property: KProperty<*>, value: DIFFICULT_TYPE) = setValueHandler(value)
